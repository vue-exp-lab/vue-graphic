...
Boid.prototype.seek = function(target) {
  var desired = this.p5.Vector.sub(target,this.position);  // A vector pointing from the location to the target
...
Boid.prototype.borders = function() {
  ...
  if (this.position.y < -this.r)  this.position.y = canvasH+this.r;
  if (this.position.x > canvasW +this.r) this.position.x = -this.r;
  if (this.position.y > canvasH+this.r) this.position.y = -this.r;
}
...
export default {
  ...
  methods: {
    setup: function(){
      ...
      for (let i = 0; i < 100; i++) {
        const b = new Boid(canvasW/2,canvasH/2, p5, sketch);
